...
const EventList: FC<{ items: SimpleEOCard[] }> = ({ items }) => {
  ...
  // slice data sesuai halaman (tetap menggunakan items.slice seperti kode asli)
  const start = (page - 1) * perPage;
  const end = start + perPage;
  const shown = combined.slice(start, end);

  // pada halaman ini, pisahkan fast dan normal
      ...
      {fastOnPage.length > 0 && (
        <section className="bg-emas/70 p-6 rounded-lg mb-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {fastOnPage.map((item) => (
              <EventCardEO key={item.id} data={item} />
            ))}
          </div>
        </section>
      )}

      {/* ==== NORMAL EVENT DI BAWAH ==== */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {normalOnPage.map((item) => (
          <EventCardEO key={item.id} data={item} />
        ))}
      </div>
